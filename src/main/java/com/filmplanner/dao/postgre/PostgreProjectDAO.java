...
public class PostgreProjectDAO implements ProjectDAO {
    ...
    public Project create(Project project) {
        ...
        try {
            ...
            if (statement.executeUpdate() > 0) { // if no rows were altered the request did nothing
                ...
                resultSet.next();
                createdProject = new Project(resultSet.getLong("id"), resultSet.getString("name"), resultSet.getString("description"));

                for (User manager : project.getManagers()) {
                    query = "INSERT INTO project_manager (project_id, user_id) VALUES (?, ?)";
                    statement = this.connection.prepareStatement(query);
                    statement.setLong(1, createdProject.getId());
    ...
    public Project create(Project project) {
        ...
        try {
            ...
            if (statement.executeUpdate() > 0) { // if no rows were altered the request did nothing
                ResultSet resultSet = statement.getGeneratedKeys();
                resultSet.next();
                Long generatedProjectId = resultSet.getLong("id");
    ...
    /* createProject
    create project inside table
    retrieve generated id

    for each manager of the project
        create relation in project_manager table

    for each shoot of the project
        create line with project's id inside shoot table

    for each paperwork of the project
        create line with project's id inside paperwork table

    create line with project's id inside client table
     */
